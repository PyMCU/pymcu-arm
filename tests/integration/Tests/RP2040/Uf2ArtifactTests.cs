...
/// <summary>
/// `pymcu build` must leave a flashable dist/firmware.uf2 beside firmware.bin:
/// it is what `pymcu flash` uploads (picotool, or drag-and-drop onto the
/// RPI-RP2 volume). Without it the ARM flash path stops at the programmer.
/// </summary>
[TestFixture]
public class Uf2ArtifactTests
{
    private const string Example = "blink";
    ...
    {
        _firmware = PymcuCompiler.BuildRp2040(Example);
    ...
    public void BlockCountAndAddressesCoverTheFlashImage()
    {
        ...
        for (int i = 0; i < _uf2.Blocks.Count; i++)
        {
            ...
            // Flat XIP image: block i lands at 0x10000000 + i * 256.
            b.TargetAddr.Should().Be(Uf2Image.XipBase + (uint)(i * Uf2Image.PayloadSize));
        }
    }

    [Test]
    public void FamilyIdIsRp2040()
    {
        _uf2.Blocks.Select(b => b.FamilyId).Should().AllBeEquivalentTo(Uf2Image.Rp2040Family);
    }
